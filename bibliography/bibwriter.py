...
def save_dict2json(json_path, dict_md5):
    with open(json_path, 'w') as fp:
        fp.write(json.dumps(dict_md5, indent=4))
...
class PublicationsGenerator:
    ...
    def __get_list_people():
        base_dir = os.getcwd()
        people_dir = '{}/bibliography/content/members'.format(base_dir)
        print(people_dir)
        list_researchers = []
    ...
    def generate_md_bibitem(self):
        ...
        out_dir = os.path.join(base_dir, '..', 'bibliography/content/publications')
        bib_file = os.path.join(base_dir, '..', 'bibliography/content/publications.bib')
        ...
        json_path = os.path.join('bibliography/content/dict_pubs.json')
    ...
    def __write_list_publications_md(self, global_index, filtered_bibkeys, string_rules):
        ...
        for bib_key in filtered_bibkeys:
            ...
            try:
                html_bibkey, only_text, bibtex, year, pub_type, pub_details = self.__append_publication_md(global_index, bib_key, html_format)
                pub_details = pub_details.replace('_', ' ').strip()
                dict_pubs[bib_key] = {}
                dict_pubs[bib_key]['html'] = html_bibkey
                dict_pubs[bib_key]['only_text'] = only_text
                dict_pubs[bib_key]['year'] = int(year)
                dict_pubs[bib_key]['pub_type'] = pub_type
                dict_pubs[bib_key]['pub_details'] = pub_details
                dict_pubs[bib_key]['bibtex'] = bibtex
    ...
    def __append_publication_md(self, global_index, bib_key, html_format):
        ...
        bibtex = bib_item.bibtex

        return pub_html, html_to_write, bibtex, year, pub_type, pub_details

    def __format_bibitem(self, bib_item, is_html_format=True):
        formatted_text = ''
        new_pubtype = None
        arxiv_link = False
        a_tags = 'data-ix="goupbox" target="_blank" class="knop footerknop movewithmouse w-button publication-button"'
        if 'doi' in bib_item.entry:
            url_doi = 'https://doi.org/' + bib_item.entry['doi']
            if is_html_format:
                formatted_text += ' <a ' + a_tags + ' href=\"' + url_doi + '\">DOI</a>'
            else:
        ...
        if 'pmid' in bib_item.entry:
            ...
            if is_html_format:
                formatted_text += ' <a ' + a_tags + ' href=\"' + url_pmid + '/\">PMID</a>'
            else:
